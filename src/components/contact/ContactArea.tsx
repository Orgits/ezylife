...
const ContactArea = ({style}: any) => {
  return (
    <>
      <section id="contact-one-page" className={`tp-contact-breadcrumb-area ${style? "pt-100" : "pt-120"} pb-120`}>
              ...
              <div className="col-lg-5">
                <div className="tp-contact-breadcrumb-wrapper">
                  {contact_data.map((item, index) => (
                    <div className={`tp-contact-breadcrumb-item d-flex ${index !== contact_data.length - 1 ? 'mb-40' : ''}`} key={index}>
                      <div className="tp-contact-breadcrumb-item-icon">
                        <span><i className={item.icon}></i></span>
                      </div>
                      <div className="tp-contact-breadcrumb-item-content">
                        <h3 className="tp-contact-breadcrumb-item-title">{item.title}</h3>
                        <a href={item.info_1} target="_blank">{item.info_1}</a>
                        {item.info_2 &&
                          <a href={item?.info_2} target="_blank">{item.info_2}</a>
                        }
                      </div>
                    </div>
                  ))}
